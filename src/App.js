import { Col, Row, Input, message, Layout, Menu } from 'antd';
...
import { classNames, ICON_COLLAPSE, ICON_EXPAND, OBJECT, ARRAY } from './components/constants';
...
class App extends Component {
  ...
  parseData = (data) => {
    ...
    try {
      ...
      this.setState({
        result: (<div dangerouslySetInnerHTML={{ __html: e.message }} />),
        hasError: true,
      });
    }
  };
  ...
  render() {
    const { result, htmlTemplate, htmlString, hasError } = this.state;
    return (
      <div className="app">
        <Layout className="layout">
          <Header>
            ...
            >
              <Menu.Item key="1">Json Visualizer</Menu.Item>
            </Menu>
          </Header>
                    ...
                    <Row type="flex" justify="space-between" align="middle">
                      <Col>
                        <span id="pasteIcon" data-clipboard-text={htmlString}>
                          <CopyOutlined title="复制到剪切板" className="html-icon" />
                        </span>
                        <DownloadOutlined title="下载" className="html-icon" onClick={this.downloadJson} />
                        { htmlTemplate ? (
                            <CloseOutlined title="取消" className="html-icon close-icon" type="close" onClick={this.hideHtmlTemplate} />
                        ) : null }
                      </Col>
